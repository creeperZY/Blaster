...
ABlasterCharacter::ABlasterCharacter()
{
	...
	Combat->SetIsReplicated(true); //combat no need to register, only need set is replicated

	Buff = CreateDefaultSubobject<UBuffComponent>(TEXT("BuffComponents"));
...
void ABlasterCharacter::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(ABlasterCharacter, OverlappingWeapon, COND_OwnerOnly); //weapon need to register
...
{
	if (bDisableGameplay) return;
	if (Combat) //only server can equip
	{
...
void ABlasterCharacter::SetOverlappingWeapon(AWeapon* Weapon)
{
	...
	if (IsLocallyControlled()) //because server doesn't have OnRep_xxx, co copy the logic from OnRep_xxx
	{
		if (OverlappingWeapon)
		{
...
void ABlasterCharacter::OnRep_OverlappingWeapon(AWeapon* LastWeapon) //lastweapon is before replicated happen, so it is the last value
{
	if (OverlappingWeapon)
	{
		OverlappingWeapon->ShowPickupWidget(true);
