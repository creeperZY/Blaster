...
FServerSideRewindResult ULagCompensationComponent::ProjectileConfirmHit(const FFramePackage& Package, ABlasterCharacter* HitCharacter, const FVector_NetQuantize& TraceStart, const FVector_NetQuantize100& InitialVelocty, float HitTime)
{
	if (HitCharacter == nullptr) return FServerSideRewindResult();
	...
	PathParams.LaunchVelocity = InitialVelocty;
	PathParams.StartLocation = TraceStart;
	PathParams.SimFrequency = 15.f;
	PathParams.ProjectileRadius = 5.f;
	PathParams.TraceChannel = ECollisionChannel::ECC_HitBox;
	...
	{
		if (PathResult.HitResult.Component.IsValid()) // we hit the head, return early
		{
...
inline void ULagCompensationComponent::EnableCharacterMeshCollision(ABlasterCharacter* HitCharacter, ECollisionEnabled::Type CollisionEnabled)
{
...
void ULagCompensationComponent::ShotgunServerScoreRequest_Implementation( const TArray<ABlasterCharacter*>& HitCharacters, const FVector_NetQuantize& TraceStart, const TArray<FVector_NetQuantize>& HitLocations, float HitTime)
{
	...
	{
		if (HitCharacter == nullptr || HitCharacter->GetEquippedWeapon() == nullptr || Controller == nullptr) continue;
